...
public:
    ...
    bool Start_RRT(std::ostream& sout, std::istream& sinput)
    {
        ...
        for (RRTNode& node : path)
        {
            ...
            _probot->GetDOFValues(dof_vals);
            _probot->GetController()->SetDesired(dof_vals);
            */
        }
        if (path.size() > 0)
        {
    ...
    RRTNode Sample(int gaussian = 1) //sampleCount is for goal-biasing
    {
        ...
        {
            //KinBody::JointPtr joint = _probot->GetJointFromDOFIndex(index);
            //bool circ = joint->IsCircular(0);
    ...
    std::vector<RRTNode> Path(NodeTree a, RRTNode q)
    {
        ...
        //while (!CompareConfig(a.GetNodes()[q.GetParent()], _startNode))
        path.push_back(q);

        /*
        cout << "START ";
        for (double& num :q.GetConfig())
            cout << "   " << num << " ";
        cout << "\n ----------------------- \n";
        */
    ...
    RRTNode NewConfig(RRTNode& q, RRTNode& qnear, NodeTree& tree)
    {
        ...
        {  //new configuration is qnear config + stepsize of DELTA in respective directions
            double elem = diff[i];
            //cout << "normalized " << elem/distance << " and adding " << (elem/distance) * DELTA << endl;
            if (DELTA < distance)
    ...
    int Extend(NodeTree& tree, RRTNode& q) //2 = reached, 1 = advanced, 0 = trapped
    {
        ...
        //check q by printing
        cout << "   q ";
        for (double& joint : q.GetConfig())
            cout << joint << " ";
        cout << endl;
        //end q check

        //check qnear by printing
        cout << "   qnear ";
        for (double& joint : qnear.GetConfig())
            cout << joint << " ";
        cout << endl;
        //end qnear check
